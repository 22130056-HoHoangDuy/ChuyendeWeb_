...
class _FruitHomeScreenState extends State<FruitHomeScreen> {
  ...
  Future<List<ProductModel>> _executeHomeSemanticSearch(String query, {int? categoryId}) async {
    ...
    if (response.statusCode == 200) {
      ...
      return rawList.map((item) => ProductModel(
        ...
        productName: item['productName'] ?? 'tên của sản phẩm',
  ...
  Widget _buildBanner() => Container(
    ...
    child: const Padding(padding: EdgeInsets.all(24), child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.center, children: [Text("Không gian riêng, niềm vui trọn vẹn.", style: TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.bold)), SizedBox(height: 4), Text("Hỗ trợ tìm kiếm thông minh bằng mô hình AI ngữ nghĩa", style: TextStyle(color: Colors.white70, fontSize: 13))])),
  );
}