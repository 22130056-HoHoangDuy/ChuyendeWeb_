...
class _SellerDashboardScreenState extends State<SellerDashboardScreen> {
  ...
  final List<String> unitList = ['Cái', 'Cây','Bộ','kg'];
  ...
  void _showDeleteConfirmation(BuildContext context, int productId) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text("Bạn có chắc chắn muốn xóa sản phẩm này không? Hành động này không thể hoàn tác và sẽ gỡ sản phẩm khỏi hệ thống tìm kiếm."),
        actions: [
  ...
  void _showEditProductDialog(BuildContext context, SellerProductItem item) {
    ...
    String selectedUnit = unitList.any((e) => e == item.unit?.trim())
        ? item.unit!.trim()
        : unitList.first;
    ...
    showDialog(
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setDialogState) => AlertDialog(
          shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
          title: const Text("Chỉnh sửa sản phẩm", style: TextStyle(fontWeight: FontWeight.bold)),
          content: SizedBox(
            width: 450,
                ...
                children: [
                  ...
                  TextField(
                    ...
                    decoration: const InputDecoration(
                      ...
                      hintText: "Cập nhật lại mô tả chi tiết sản phẩm...",
                    ),
                  ),
        ...
        builder: (context, setDialogState) => AlertDialog(
          ...
          title: const Row(
            children: [
              ...
              SizedBox(width: 10),
              Text("Đăng Bán sản phẩm mới", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18)),
            ],
          ),
          content: SizedBox(
            width: 450,
            ...
            ElevatedButton(
              ...
              onPressed: (_isSaving || selectedCategoryId == null || tempImageUrl.isEmpty) ? null : () async {
                ...
                if (success && context.mounted) {
                  await _triggerSync();
                  Navigator.pop(context);
                  ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Đăng bán sản phẩm thành công!")));
                  setState(() {
                    _dashboardFuture = SellerRemoteDataSource().getDashboardData(widget.accessToken);
                  });
                } else if (context.mounted) {
                  ...
                  children: [
                    ...
                    GridView.count(
                      ...
                      children: [
                        ...
                        _statCard("SẢN PHẨM CẢNH BÁO KHO", "${data.lowStockProducts} mặt hàng", Icons.warning_amber_rounded, Colors.orange),
                      ],
                    ),
          ...
          Row(
            ...
            children: [
              ...
              ElevatedButton.icon(
                ...
                label: const Text("THÊM SẢN PHẨM", style: TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.bold)),
              )
            ],
          ),
